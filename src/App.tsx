...
function App() {
  // Auth Hook
  const { user, authLoading, error: authError, loginWithGoogle, logout } = useAuth();
  ...
  const {
    ...
    corrections, toneSuggestions, styleSuggestions,
    languageStyleMixing, punctuationIssues, euphonyImprovements, contentAnalysis,
    stats, userDictionary,

    // Actions
    saveSettings, toggleSection,
    handleHighlight, handleReplace, dismissSuggestion,
    checkSpelling, shouldShowSection,
    handleAddToDictionary, handleRemoveFromDictionary,

    // Helpers
    getToneName, getDocTypeLabel,
    TONE_OPTIONS, STYLE_OPTIONS, DOC_TYPE_CONFIG
  } = useBhashaMitra();
  ...
  // 3. Authenticated State (Show Main App)
  return (
    <div className="app-container">
      ...
      <div className="header-section">
        <div className="header-top">
          ...
          <div className="app-title">
            <h1>🌟 ভাষা মিত্র</h1>
            <p>বাংলা বানান পরীক্ষক</p>
          </div>
          {/* User Avatar / Logout */}
          <div className="user-profile" onClick={logout} title="লগআউট করতে ক্লিক করুন">
            {user.photoURL ? (
      ...
      <div className="content-area">
        ...
        {!isLoading && stats.totalWords === 0 && !message && (
          <div className="empty-state">
            ...
            <p>স্বাগতম, {user.displayName?.split(' ')[0]}!</p>
            <p style={{fontSize: '11px', marginTop: '5px'}}>সাজেশন পেতে টেক্সট সিলেক্ট করে 'পরীক্ষা করুন' চাপুন</p>
          </div>
        )}
        ...
        <div className="modal-overlay" onClick={() => setActiveModal('none')}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="menu-profile-info" style={{marginBottom: '15px', padding: '10px', background: '#f3f4f6', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '10px'}}>
                {user.photoURL && <img src={user.photoURL} alt="User" style={{width: '30px', height: '30px', borderRadius: '50%'}} />}
                <div>
                  <div style={{fontWeight: 'bold', fontSize: '12px'}}>{user.displayName}</div>
                  <div style={{fontSize: '10px', color: '#6b7280'}}>{user.email}</div>
                </div>
              </div>

              <div className="option-item" onClick={() => setActiveModal('dictionary')}>
                <div className="opt-icon">📕</div>
                <div><div className="opt-title">আমার ডিকশনারি</div><div className="opt-desc">আপনার সেভ করা শব্দ দেখুন</div></div>
              </div>
              ...
              <button onClick={logout} className="btn-primary-full" style={{marginTop: '10px', background: '#ef4444'}}>লগ আউট</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Other Modals (Instructions, Tone, Style, DocType) - Same as before */}
      {/* Instructions */}
